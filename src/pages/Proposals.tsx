...
export default function Proposals() {
  ...
  const { proposals, updateStatus, deleteProposal, getProposalsByRepresentative } = useProposalsStore();

  // Obter apenas as propostas do representante atual
  const representativeProposals = getProposalsByRepresentative(user?.id || '');

  // Filtrar propostas
  const filteredProposals = representativeProposals.filter(proposal => {
    const matchesSearch = 
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-semibold text-gray-900">Minhas Propostas</h1>
