...
CREATE POLICY "profiles_select_20250103_v2"
  ON public.profiles
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY "profiles_update_20250103_v2"
  ON public.profiles
  FOR UPDATE
  TO authenticated
...
CREATE POLICY "profiles_admin_20250103_v2"
  ON public.profiles
  FOR ALL
  TO authenticated
  USING (is_admin())
...
-- Create indexes
CREATE INDEX idx_profiles_auth_id_v2 ON profiles(auth_id);
CREATE INDEX idx_profiles_role_v2 ON profiles(role);
CREATE INDEX idx_profiles_email_v2 ON profiles(email);
